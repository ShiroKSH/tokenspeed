...
2. (removed) the racy Triton megafusion, deleted from the tree — parity now
   ...
   exceeds one wave (measured as run-to-run nondeterminism from B=192 at the
   K3 TP8 geometry, grid 9216). The CuTe kernel orders the shift with a
   cluster barrier instead. Its determinism is covered by repeat-and-compare
   sweeps rather than by a check in this file -- nv=4 is unreachable from the
   dispatcher (``_pick_nv`` returns 1 or 2), so there is no nv=2 vs nv=4
   comparison here despite what this note used to claim.
